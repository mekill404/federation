...
public class CollectivityRepository {
    ...
    public void addMembership(String collectivityId, String memberId, String role) {
        String sql = "INSERT INTO membership (member_id, collectivity_id, role_in_collectivity, joined_at) VALUES (?::uuid, ?::uuid, ?::member_occupation, ?)";
        try (Connection conn = dbConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
    ...
    public List<MemberEntity> getMembers(String collectivityId) {
        ...
        String sql = "SELECT m.* FROM member m JOIN membership ms ON m.id = ms.member_id WHERE ms.collectivity_id = ?::uuid AND ms.left_at IS NULL";
        try (Connection conn = dbConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, collectivityId);
    ...
    public boolean isMemberOfCollectivity(String memberId, String collectivityId) {
        String sql = "SELECT 1 FROM membership WHERE member_id = ?::uuid AND collectivity_id = ?::uuid AND left_at IS NULL";
        try (Connection conn = dbConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, memberId);
            pstmt.setString(2, collectivityId);
    ...
    private void loadStructure(CollectivityEntity col) {
        String sql = "SELECT * FROM collectivity_structure WHERE collectivity_id = ?::uuid ORDER BY mandat_year DESC LIMIT 1";
        try (Connection conn = dbConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, col.getId());
            try (ResultSet rs = pstmt.executeQuery()) {
    ...
    public void updateIdentifiers(String collectivityId, String uniqueNumber, String uniqueName) {
        String sql = "UPDATE collectivity SET unique_number = ?, unique_name = ? WHERE id = ?::uuid";
        try (Connection conn = dbConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, uniqueNumber);
